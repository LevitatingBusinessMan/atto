...
pub fn setup_logging(args: &crate::Args) -> io::Result<()> {
    ...
    let syslog_layer = {
        ...
        syslog_tracing::Syslog::new(IDENTITY, options, facility).map(|writer| {
    ...
    let file_layer = {
        let file = dirs::cache_dir()
            .and_then(|cache_dir| fs::File::options()
                .write(true)
                .append(true)
                .create(true)
                .open(cache_dir.join("atto.log"))
                .ok()
            );
        file.map(|file| tracing_subscriber::fmt::layer()
